...
async def update_invoice(community_id, subscribtion_invoice_id, **data):
    ...
    invoice = await subscription.find_one(
        subscription.subscription_invoice_id == subscribtion_invoice_id
    )
    if invoice is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
...
async def generate_invoice(community_id, subscribtion_invoice_id, url):
    comm = await subscription.find_one(
        subscription.community_id == community_id
    )
    if comm is None:
...
# Download invoice
async def download_invoice(community_id, subscribtion_invoice_id):
    comm = await subscription.find_one(
        subscription.community_id == community_id
    )
    if comm is None:
